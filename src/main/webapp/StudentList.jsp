	...
	<div class="wrapper-content">
		<div class="header-btn">
			...
			<form class="search-form"
				action="<%=request.getContextPath()%>/Student/search" method="post">
			...
			<table class="data" width='100%'>
				...
				<tbody>
					...
					<tr>
						...
						<td><%=studentList.get(i).getGender() ? "Nữ" : "Nam"%></td>
						...
						%>
						<td><a
							href="<%=request.getContextPath()%>/Student/update?id=<%=studentList.get(i).getId_person()%>"><button
									type="button">
									<i class="update-icon fa-solid fa-pen-to-square"></i>
								</button></a></td>
						<td><a
							href="<%=request.getContextPath()%>/Student/delete?id=<%=studentList.get(i).getId_person()%>"><button
									type="button">
									<i class="update-icon fa-solid fa-trash"></i>
								</button></a></td>
						<%
						}
						%>
			...
			<div class="container scroll">
				...
				<div class="modal-body">
					<div class="modal-form">
						<form action="<%=request.getContextPath()%>/Student/add"
							method="post">
							<label for="name" class="modal-label">Tên Học Sinh</label> <br>
							<input type="text" name="name" placeholder="Nhập tên học sinh"
								class="modal-input" required /> <label for="password"
								class="modal-label">Mật Khẩu</label> <br> <input
								type="password" name="password" placeholder="Nhập mật khẩu"
								class="modal-input" required /> <label for="phone"
								class="modal-label">SĐT</label> <br> <input type="text"
								name="phone" placeholder="Nhập SĐT" class="modal-input" required />
							<label for="email" class="modal-label">Email</label> <br> <input
								type="text" name="email" placeholder="Nhập email"
								class="modal-input" required /> <label for="cccd"
								class="modal-label">CCCD</label> <br> <input type="text"
								name="cccd" placeholder="Nhập email" class="modal-input"
								required /> <label for="gender" class="modal-label">Giới
								Tính</label> <br> <select name="gender" class="modal-option">
								<option value=0 selected>Nam</option>
								<option value=1>Nữ</option>
							</select> <label for="address" class="modal-label">Địa Chỉ</label> <br>
							<input type="text" name="address" placeholder="Nhập địa chỉ"
								class="modal-input" required /> <label for="date"
								class="modal-label">Ngày Sinh</label> <br> <input
								type="date" name="dob" placeholder="Chọn ngày sinh"
								class="modal-input" required /> <label for="id_role"
								class="modal-label">Lớp</label> <br> <input type="text"
								name="id_role" placeholder="Chọn lớp" class="modal-input"
								required />
