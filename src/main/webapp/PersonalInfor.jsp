    ...
    <div class="wapper-login">
		...
		<form action="./PersonalInformation" method="post">
			...
			/* if(role.equals("admin")) {
				AdminView person = (AdminView) request.getAttribute("admin");
			}
			else if (role.equals("student")){
				StudentView person = (StudentView) request.getAttribute("student");
			} */
			...
			<label for="id_person" class="modal-label">Mã số</label>
			<br>
			<input type="text" name="id_person" value="<%= role.equals("admin") ? adminView.getId_person(): (role.equals("student") ? student.getId_person() :lecturerView.getId_person())%>" class="modal-input" readonly />
			</div>
			<div class="modal-form-element">
			<label for="name" class="modal-label">Tên</label>
			<br>
			<input type="text" name="name" value="<%= role.equals("admin") ? adminView.getName(): (role.equals("student") ? student.getName() :lecturerView.getName()) %>" class="modal-input"/>
			</div>
			</div>
			<div class="modal-form-item">
			<div class="modal-form-element">
			...
			<br>
			<input type="text" name="phone" value="<%=role.equals("admin") ? adminView.getPhone(): (role.equals("student") ? student.getPhone() :lecturerView.getPhone())%>" class="modal-input" />
			</div>
			<div class="modal-form-element">
			<label for="email" class="modal-label">Email</label>
			<br>
			<input type="text" name="email" value="<%=role.equals("admin") ? adminView.getEmail(): (role.equals("student") ? student.getEmail() :lecturerView.getEmail())%>" class="modal-input" />
			</div>
			</div>
			<label for="email" class="modal-label">CCCD</label>
			<br>
			<input type="text" name="cccd" value="<%=role.equals("admin") ? adminView.getCCCD(): (role.equals("student") ? student.getCCCD() :lecturerView.getCCCD())%>" class="modal-input"/></td>
			<label for="gender" class="modal-label">Giới Tính</label>
			<br>
			<select name="gender" class = "modal-option">
						<%
						if (!(role.equals("admin") ? adminView.getGender(): (role.equals("student") ? student.getGender() :lecturerView.getGender()))) {
						%>
			...
			<br>
			<input type="text" name="address" value="<%=role.equals("admin") ? adminView.getAddress(): (role.equals("student") ? student.getAddress() :lecturerView.getAddress())%>" class="modal-input"/></td>
			<label for="date" class="modal-label">Ngày Sinh</label>
			<br>
			<input type="date" name="dob" value="<%=role.equals("admin") ? adminView.getDob(): (role.equals("student") ? student.getDob() :lecturerView.getDob())%>" class="modal-input"/>
			...
			<select name="id_role" class = "modal-option">
				...
				<% } else {%>
					...
					<select name="id_faculty" class ="modal-option" required>
        				...
        				<br>
        				<input type="text" name="lecturer_salary" value="<%= role.equals("admin") ? adminView.getAdmin_salary():lecturerView.getLecturer_salary()%>" class="modal-input">
        			<% }%>
