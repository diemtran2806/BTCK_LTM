...
<body>
	...
	<div class="wrapper-content">
		<div class="header-btn">
			...
			<form class="search-form" name="search_" method="post"
				action="./viewlist">
				<input <%if (request.getAttribute("keysearch") != null) {%>
					value="<%=request.getAttribute("keysearch")%>" <%}%>
					class="search" type="text" name="search" placeholder="Type here..."
					value="">
				<button type="submit" class="btn">Search</button>
			</form>
		</div>
		<form name="delete_form" method="post" action="./delete"
			onsubmit="return requiredDelete()">
			<table class="data" width='100%'>
				...
				<tbody>
					...
					<tr>
						...
						if (session.getAttribute("role") != null && session.getAttribute("role").equals("admin")) {
						%>
						<td><input type="checkbox" class="delete" name="delete"
							value="<%=lecturerList.get(i).getId_person()%>"></td>
						<%
						}
						%>
						<td><div class="view-img"><img alt="default" width="42" height="42"
							src="<%=request.getContextPath()%>/public/img/<%=lecturerList.get(i).getImg()%>"></div></td>
						...
						%>
						<td><a
							href="./update?id=<%=lecturerList.get(i).getId_person()%>"><button
									type="button">
			...
			<div class="container scroll">
				...
				<div class="modal-body">
					<div class="modal-form">
						<form name="create_emp" method="post" action="./new"
							onsubmit="return required()">
							<label for="name" class="modal-label">Tên Giảng Viên</label> <br>
							<input type="text" name="name" value=""
								placeholder="Nhập tên giảng viên" class="modal-input"> <label
								for="password" class="modal-label">Mật Khẩu</label> <br> <input
								type="password" name="password" placeholder="Nhập mật khẩu"
								class="modal-input" value=""> <label for="role"
								class="modal-label">Chức Vụ</label> <br> <input type="text"
								name="role" placeholder="Nhập chức vụ" class="modal-input"
								value=""> <label for="phone" class="modal-label">SĐT</label>
							<br> <input type="text" name="phone" placeholder="Nhập SĐT"
								class="modal-input" value=""> <label for="email"
								class="modal-label">Email</label> <br> <input type="email"
								name="email" placeholder="Nhập SĐT" class="modal-input" value="">
							<label for="CCCD" class="modal-label">CCCD/CMND</label> <br>
							<input type="text" name="CCCD" placeholder="Nhập số cmnd/cccd"
								class="modal-input" value=""> <label for="gender"
								class="modal-label">Giới Tính</label> <br> <select
								name="gender" class="modal-option" required>
								<option selected value="0">Nam</option>
								<option value="1">Nữ</option>
							</select> <label for="address" class="modal-label">Địa Chỉ</label> <br>
							<input type="text" name="address" placeholder="Nhập địa chỉ"
								class="modal-input" value=""> <label for="date"
								class="modal-label">Ngày Sinh</label> <br> <input
								type="date" name="dob" placeholder="Nhập ngày sinh"
								class="modal-input" value=""> <label for="id_faculty"
								class="modal-label">Chọn Khoa</label> <br> <select
								name="id_faculty" class="modal-option" required>
							...
							<input type="text" name="lecturer_salary"
								placeholder="Nhập lương" class="modal-input" value="">
